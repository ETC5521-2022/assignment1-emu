...
world_cap_farmlong <- world_cap_farm %>% 
        pivot_wider(names_from = production_type, values_from= production )%>%
        mutate(total= aquaculture+ captured)%>%
        filter(Year== 2015)
  
avg_contribution2015 <-   avg_contribution1 %>%
        mutate(contribution= total/world_cap_farmlong$total)
