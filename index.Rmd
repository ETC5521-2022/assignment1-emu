...
In addition, the use of sorted data from the original report help us quite a lot to simplify the frocess. These data includes: 'country_cf' , 'world_cap_farm' and 'captured_vs_farmed_long'.
...
captured_vs_farmed_long  <- captured_vs_farmed %>% pivot_longer(cols = aquaculture:captured,names_to = "production_type",values_to = "production")
...
Graph.5g <- ggplot(avg_contribution2015,aes(x=reorder(Entity, (contribution)), y=contribution, fill=Entity))+
  geom_col()+
   theme(axis.text.x = element_text(angle = 60, hjust = 1))+
theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(x = "Percentage contribution to world's seafood production", y = "Entity")+
  ggtitle("Graph.5g: Contribution of top 10 seafood producers 2010s")+
  coord_flip()
...
It can be seen from the chart that the top 2 producers contribute over 50% of the total seafood supply.
...
5 China and Indonesia are the top 2 seafood producers in total production as well as the farmed and captured ranking. Over the last 60 years, these two countries experenced the most significant upward trend in seafood production.
...
stock_worldL <- stock_world %>% pivot_longer(cols = Sustainable_levels_Fish	:Overexploited_Fish,names_to = "type", values_to = "proportion") 
